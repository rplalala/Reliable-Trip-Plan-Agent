...
Blocked by: Live verification requires separate authorization.
...
http://geodb-free-service.wirefreethought.com/v1/geo/places, with types=CITY. No RapidAPI
subscription, key, authentication headers or paid fallback. Browser calls only our backend;
only the backend-to-GeoDB hop uses HTTP. Send no preferences, trip facts or credentials.
...
2. API admission: short/long input, fixed HTTP host and no auth headers, result cap, 429/timeout/error
